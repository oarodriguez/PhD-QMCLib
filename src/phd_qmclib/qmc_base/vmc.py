...
class StateData(t.NamedTuple):
    """Represent the data necessary to execute the VMC states generator."""
    sys_conf: np.ndarray
...
class SamplingBase(metaclass=ABCMeta):
    ...
    def states(self, ini_state: State) -> t.Iterator[State]:
        ...
        :param ini_state: The initial configuration of the sampling.
        """
        return self.core_funcs.states_generator(ini_state,
                                                self.rng_seed,
                                                self.cfc_spec)
...
class CoreFuncs(metaclass=ABCMeta):
    ...
    def get_state_data_item(self):
        ...
        def _get_state_data(source: StateData,
            ...
            :return:
            """
            sys_confs = source.sys_conf[item]
            return StateData(sys_conf=sys_confs)
    ...
    def states_generator(self):
        ...
        def _states_generator(ini_state: State,
                              rng_seed: int,
                              cfc_spec: CFCSpec):
            """VMC sampling generator.
            ...
            base_shape = ()
            aux_next_state_data = init_state_data(base_shape, cfc_spec)
    ...
    def blocks(self):
        ...
        :return: The JIT compiled function that execute the Monte Carlo
            integration.
        """
        generator = self.states_generator
    ...
    def as_chain(self):
        ...
        :return: The JIT compiled function that execute the Monte Carlo
            integration.
        """
        confs_props_blocks = self.state_data_blocks

        @jit(nopython=True, nogil=True)
        def _as_chain(num_steps: int,
                      ini_state: State,
                      rng_seed: int,
                      cfc_spec: CFCSpec) -> SamplingStateDataBlock:
    ...
    def state_data_blocks(self):
        ...
        @jit(nopython=True, nogil=True)
        def _state_data_blocks(num_steps_block: int,
                               ini_state: State,
                               rng_seed: int,
                               cfc_spec: CFCSpec):
            """Returns the VMC sampling blocks of states configurations.
            ...
            """
            ns_block = num_steps_block
            sdb_shape = pb_shape = ns_block,
            state_data_blocks = init_state_data(sdb_shape, cfc_spec)
            props_block = init_props_data_block(pb_shape)

            state_confs_block = state_data_blocks.sys_conf
            wf_abs_log_set = props_block.wf_abs_log
            move_stat_set = props_block.move_stat

            states_gen = states_generator(ini_state, rng_seed, cfc_spec)
            ...
            while True:
                ...
                # enum_iter: T_E_SIter = enumerate(states_gen)
                ...
                for state in states_gen:
                    ...
                    state_data = \
                        get_state_data_item(state_data_blocks, step_idx)
                    wf_abs_log = state.wf_abs_log
                    move_stat = bool(state.move_stat)

                    copy_state_data(state, state_data)
                    wf_abs_log_set[step_idx] = wf_abs_log
                    move_stat_set[step_idx] = move_stat
                    accepted += move_stat
                    ...
                    if step_idx + 1 >= ns_block:
                        ...
                        block_data = \
                            SamplingStateDataBlock(state_confs_block,
                                                   props_block,
                                                   accept_rate,
                                                   last_state=last_state)
                        yield block_data
